...
6. 外部IDは `provider + providerSubject` の一意制約を持つ専用モデルとし、`IdentityAccess` を介してUserプロフィールへ関連付ける
...
```prisma
AuthIdentity 1 -- * IdentityAccess * -- 1 User
```

この中間モデルにより、保護者のGoogle IDへ本人のparentプロフィールと子どものstudentプロフィールを別々の権限として付与できる。Phase 1では既存Userとの明示連携とデフォルトプロフィールでのログインまでを有効化し、guardian付与・プロフィール切替UIはまだ有効化しない。
...
1. 外部ID・アクセス権モデル、監査ログを追加（実装済み）
2. 既存利用者向けGoogle連携をリリース（実装済み）
3. 招待受理中のGoogle認証を追加
4. 先生のGoogle新規登録を追加
