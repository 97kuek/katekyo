...
| プロフィール | `/profile` | 表示名・パスワード変更。不要なリダイレクトやDB参照を挟まない |
| 設定 | `/settings` | Google・LINE連携 / Meet URL / 科目タグ管理（`/subjects` はここへリダイレクト） |
...
**その他:** 利用頻度と目的で「学習管理（または学習）」→「アカウント（プロフィール / 設定）」→「サポート（使い方ガイド）」に分類する
