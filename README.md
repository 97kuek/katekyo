...
- 推移グラフ（折れ線）で学習の伸びを可視化
- 同じテストの科目別バランスをレーダーチャートで表示
...
スマートフォンでの利用を主眼に、ネイティブアプリに近い操作感を実装。

- ホーム画面に追加して全画面で起動（PWA）。iOS の safe-area に対応し、ボトムナビ・コンテンツがホームインジケーターに被らない
- 宿題・成績の一覧カードは**左スワイプで編集/削除**、左端まで振り切ると削除（`navigator.vibrate` による触覚フィードバック付き）
- 主要リストの**プルダウン更新**、長いフォームの**送信ボタンを画面下部に固定**
- 入力欄は 16px ベースでフォーカス時の自動ズームを防止
